...
function CardLabel(props: CardFloatingLabelInputProps) {
  return (
    <label htmlFor={props.inputId} className="pl-2 absolute text-base bg-branco-gelo text-fundo-claro duration-300 transform
...
function CardFloatingLabelInput(props: CardFloatingLabelInputProps) {
  return (
      <div className="relative mx-[3.625rem]">
        <input type="text" id={props.inputId} name={props.inputId} className={`block px-2.5 pb-2.5 pt-4 text-base
        text-fundo-claro bg-transparent rounded-lg border-[0.1rem] border-fundo-claro appearance-none
        focus:outline-none focus:ring-0 focus:border-2 focus:border-azul-ufal peer ${props.width}`} placeholder=" " />
        <CardLabel label={props.label} inputId={props.inputId} width={props.width} />
      </div>
  )
}
