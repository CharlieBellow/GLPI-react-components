...
export function Pagination({
  ...
  const nextPages = currentPage < lastPage
    ? generatePageArrays(currentPage, Math.min(currentPage + siblingsCount, lastPage))
    : [];
  ...
  return (
    ...
    >
      <Box>
        <strong>{((currentPage - 1) * registersPerPage) + 1} </strong> - <strong>{totalCountOfRegisters < (registersPerPage * currentPage) ? totalCountOfRegisters : registersPerPage * currentPage}</strong> de <strong> {totalCountOfRegisters} </strong>
      </Box>
